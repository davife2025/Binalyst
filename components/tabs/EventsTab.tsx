  ...
  return (
    <div className="max-w-4xl mx-auto px-6 py-6">
      ...
      <div className="flex items-start justify-between mb-6 gap-4 flex-wrap">
        <div>
          <h2 className="text-lg font-extrabold" style={{ color: 'var(--text)' }}>Events Radar</h2>
      ...
      {/* Error */}
      {error && (
        <div className="rounded-xl p-3 mb-4 mono text-xs" style={{ background: 'rgba(246,70,93,0.08)', border: '1px solid rgba(246,70,93,0.25)', color: 'var(--red)' }}>
          {error}
        </div>
      )}
        ...
        <div className="flex gap-2 mb-4 flex-wrap">
          {['all', 'listing', 'trading', 'airdrop', 'launchpool', 'other'].map(f => (
            <button key={f} onClick={() => setFilter(f)}
              className="mono text-xs px-3 py-1.5 rounded-full transition-all capitalize"
              style={{
      ...
      {filtered.length > 0 ? (
        <div className="flex flex-col gap-3">
          {filtered.map(ev => {
            ...
            return (
              <div key={ev.id} className="flex items-center gap-4 rounded-xl p-4 transition-all"
                style={{ background: 'var(--bg2)', border: '1px solid var(--border)' }}>
                <div className="w-10 h-10 rounded-xl flex items-center justify-center text-xl shrink-0" style={{ background: cfg.bg }}>{cfg.icon}</div>
