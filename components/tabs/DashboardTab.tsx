 ...
 * BNB brand aesthetic: black bg, gold grid overlay, dot matrix, bold uppercase type.
...
const QA_ITEMS = [
  ...
  { tab: 'competition', label: 'Competition',      desc: 'Live PnL + trade log',        iconColor: '#F0B90B',
    icon: <><path d="M6 9H4.5a2.5 2.5 0 0 1 0-5H6"/><path d="M18 9h1.5a2.5 2.5 0 0 0 0-5H18"/><path d="M18 2H6v7a6 6 0 0 0 12 0V2Z"/><path d="M4 22h16"/></> },
...
const PILLARS = [
  ...
  },
  {
    ...
    desc: 'BNB Chain, Celo, Mantle, Sui, X Layer and more. On-chain proof of every trade. Registered agent identity across networks.',
...
export default function DashboardTab() {
  ...
  const steps = [
    ...
    { label: 'Register Agent',    done: session?.isRegistered ?? false, tab: 'agent-wallet' },
    { label: 'Start Agent',       done: agentStatus === 'running',   tab: 'competition'  },
  ]
  ...
  const agentChain = network === 'mainnet' ? 'BSC Mainnet' : 'BSC Testnet'

  return (
    <div className="flex-1 overflow-y-auto" style={{ background: '#000', position: 'relative' }}>

      {/* BNB-brand grid overlay */}
      <div style={{
      ...
      <div className="max-w-5xl mx-auto px-6 py-6 flex flex-col gap-5" style={{ position: 'relative', zIndex: 1 }}>
        ...
        <div className="grid gap-3" style={{ gridTemplateColumns: '1fr 1.7fr 1fr' }}>
          ...
          {/* Agent status — BNB Chain badge shown here because agent runs on it */}
          <div className="rounded-md p-4 flex flex-col gap-3"
            style={{ background: '#0F0F0F', border: '1px solid rgba(240,185,11,.12)' }}>
            <div className="mono uppercase tracking-widest font-bold" style={{ fontSize: 8, color: 'rgba(255,255,255,.22)' }}>
              Agent Status
