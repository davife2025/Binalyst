...
type Tab = 'chat' | 'markets' | 'events' | 'learn' | 'portfolio' | 'trading' | 'alerts' | 'agent' | 'web3' | 'settings'

const NAV: { id: Tab; label: string; icon: string; dot?: boolean }[] = [
  ...
  { id: 'web3',      label: 'Web3',       icon: '🌐', dot: true },
]
        ...
        {NAV.map(item => {
          ...
          return (
            <button key={item.id} onClick={() => setActiveTab(item.id)}
              ...
              {item.dot && <span className="w-1.5 h-1.5 rounded-full shrink-0" style={{ background: item.id === 'web3' ? 'var(--yellow)' : 'var(--green)', animation: 'blink 2s infinite' }} />}
            </button>
          )
        })}

        {/* Skills badge */}
        <div className="mt-2 mx-1 px-3 py-2 rounded-lg" style={{ background: 'var(--bg3)', border: '1px solid rgba(240,185,11,0.15)' }}>
          <div className="mono text-[9px] uppercase tracking-widest mb-1" style={{ color: 'var(--text3)' }}>Skills Hub</div>
          <div className="flex flex-wrap gap-1">
            {['Alpha','Square','Token','Audit','Wallet','MemeRush','Rank'].map(s => (
              <span key={s} className="mono text-[8px] px-1.5 py-0.5 rounded" style={{ background: 'rgba(240,185,11,0.08)', color: 'var(--yellow)', border: '1px solid rgba(240,185,11,0.15)' }}>{s}</span>
            ))}
          </div>
        </div>
      </nav>
