...
export const mantleAgentIdentitySkill: ByrealSkill = {
  name:        'mantle_agent_identity',
  description: 'Get ERC-8004 agent identity info for a Mantle agent. Returns agentId, 8004scan URL, and network. Turing Test Hackathon feature #2.',
  version:     '1.0.0',
  author:      'Binalyst',
  tags:        ['mantle', 'erc8004', 'identity', 'nft', 'turing-test'],
  input: [
    { name: 'agentId', type: 'string', description: 'ERC-8004 agent token ID', required: true },
  ],
  output: [
    { name: 'agentId', type: 'string', description: 'ERC-8004 token ID',                    required: true },
    { name: 'scanUrl', type: 'string', description: '8004scan.io URL for this agent',        required: true },
    { name: 'network', type: 'string', description: 'Network the identity is registered on', required: true },
  ],
  handler: async ({ agentId }) => withTiming('mantle_agent_identity', async () => {
    const id      = String(agentId)
    const scanUrl = `https://8004scan.io/agents/mantle/${id}`
    return { agentId: id, scanUrl, network: 'mantle-mainnet' }
  }),
}
