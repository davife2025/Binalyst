 ...
 * Using Kimi K2.5 (Moonshot AI) — free, OpenAI-compatible, 256k context.
...
const TOOLS: OpenAI.ChatCompletionTool[] = [
  ...
  { type: 'function', function: { name: 'skill_market_rank', description: 'Get crypto market rankings: trending, smart money, social hype, meme tokens, top traders.', parameters: { type: 'object', properties: { rankType: { type: 'string', enum: ['trending','smart_money','social_hype','meme','alpha','traders'] }, chainId: { type: 'string' }, size: { type: 'number' } } } } },
...
const SYSTEM: Record<AgentMode, string> = {
  assistant: `You are Binalyst, an elite AI assistant for Binance users. You have live access to Binance market data and 8 Binance Skills Hub integrations. Use tools to get real data — never guess prices. Be concise and data-driven. Use **bold** for prices and key metrics.`,
...
async function executeTool(name: string, args: any, credentials?: BinanceCredentials): Promise<any> {
  ...
  switch (name) {
    ...
    case 'get_klines': {
      ...
      return { symbol: args.symbol, currentPrice: closes[closes.length-1], high: Math.max(...klines.map((k: any) => parseFloat(k.high))), low: Math.min(...klines.map((k: any) => parseFloat(k.low))), change: ((closes[closes.length-1]-closes[0])/closes[0]*100).toFixed(2)+'%' }
    }
    ...
    case 'skill_address_info': {
      const [info, holdings] = await Promise.allSettled([getAddressInfo({ address: args.address, chainId: args.chainId ?? '56' }), getAddressTokenHoldings({ address: args.address, chainId: args.chainId ?? '56' })])
