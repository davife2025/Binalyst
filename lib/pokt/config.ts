/**
 * lib/pokt/config.ts — ENDPOINT FIXES (replaces Session P1 version)
 *
 * Corrections applied:
 *   - arbitrum-one.api.pocket.network → arb-one.api.pocket.network
 *   - optimism.api.pocket.network     → op.api.pocket.network
 *   - gnosischain-mainnet.api.pocket.network → gnosis.api.pocket.network
 *   - harmony removed entirely (not a canonical Pocket Network chain)
 *   - rpcUrlKeyed / POKT_GATEWAY_KEY removed — Grove/Nodies gateway pattern
 *     does not apply to the POKT public portal; all public endpoints are
 *     accessed directly at <chain>.api.pocket.network with no key
 *
 * Verified public RPC base URLs (June 2026):
...
export const POKT_CHAINS: Record<string, POKTChain> = {
  ...
  optimism: {
    ...
    rpcUrl:   'https://op.api.pocket.network',
  ...
  gnosis: {
    ...
    rpcUrl:   'https://gnosis.api.pocket.network',
...
export const POKTSCAN_API = {
  ...
  metrics:    'https://poktscan.com/api/metrics',
...
export const POKT_TOKENOMICS = {
  ...
  SUPPLIER_SHARE_PCT:      79,
  VALIDATOR_SHARE_PCT:     14,
  DAO_SHARE_PCT:           4.5,
  SOURCE_OWNER_SHARE_PCT:  2.5,
  SUPPLIER_MIN_STAKE:      60_000,
  VALIDATOR_MIN_STAKE:     1_000_001,
  DELEGATOR_MIN_STAKE:     1,
  APP_MIN_STAKE:           1_000,
  UNBONDING_DAYS:          21,
  CIRCULATING_SUPPLY:      2_300_000_000,
}
